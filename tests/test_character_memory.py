...
class TestMemoryType(unittest.TestCase):
    ...
    def test_memory_type_values(self):
        ...
        self.assertEqual(
            MemoryType.SHARED_EXPERIENCE.value, "shared_experience"
        )
        self.assertEqual(MemoryType.RECURRING_JOKE.value, "recurring_joke")
        self.assertEqual(
            MemoryType.UNFINISHED_ACTIVITY.value, "unfinished_activity"
        )
        self.assertEqual(
            MemoryType.RELATIONSHIP_PROGRESSION.value,
            "relationship_progression",
        )
        self.assertEqual(
            MemoryType.PERSISTENT_DECISION.value, "persistent_decision"
        )
...
class TestCharacterMemoryDatabase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_store_and_get_memory(self):
        ...
        self.assertEqual(
            memories[0].content, "We promised to play chess next week."
        )
...
class TestBuildCharacterMemoryContext(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.test_db = os.path.join(
            self.temp_dir, "test_character_memory_context.db"
        )
    ...
    async def test_build_context_with_memories(self):
        ...
        self.assertIn(
            "[RECURRING_JOKE] The rubber duck incident. (importance: 0.5)",
            result,
        )
...
class TestExtractionPipeline(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("utils.providers.generate_text")
    @patch("utils.conversation.build_conversation_context")
    async def test_extract_memories_empty_response(
        self, mock_build_context, mock_generate
    ):
    ...
    @patch("utils.providers.generate_text")
    @patch("utils.conversation.build_conversation_context")
    async def test_extract_memories_filters_low_importance(
        self, mock_build_context, mock_generate
    ):
...
class TestEnforceMemoryBudget(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        self.temp_dir = tempfile.mkdtemp()
        self.test_db = os.path.join(
            self.temp_dir, "test_character_memory_budget.db"
        )
