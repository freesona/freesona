...
class TestPromptBuilderArchitecture(unittest.TestCase):
    ...
    def test_get_provider_metadata_returns_correct_info(self):
        ...
        char_meta = next(
            m for m in metadata if m["name"] == "character_memory"
        )
        self.assertEqual(
            char_meta["priority"], ProviderPriority.CHARACTER_MEMORY
        )
        self.assertEqual(char_meta["mutability"], Mutability.MUTABLE.value)

        # Check persona_knowledge_base provider metadata (immutable)
        kb_meta = next(
            m for m in metadata if m["name"] == "persona_knowledge_base"
        )
        self.assertEqual(
            kb_meta["priority"], ProviderPriority.PERSONA_KNOWLEDGE_BASE
        )
...
class TestContextBlock(unittest.TestCase):
    ...
    def test_is_empty_property(self):
        ...
        self.assertTrue(
            ContextBlock("a", 1, Mutability.IMMUTABLE, "").is_empty
        )
        self.assertTrue(
            ContextBlock("a", 1, Mutability.IMMUTABLE, "   ").is_empty
        )
        self.assertFalse(
            ContextBlock("a", 1, Mutability.IMMUTABLE, "content").is_empty
        )

    def test_len(self):
        """len() should return content length."""
        self.assertEqual(
            len(ContextBlock("a", 1, Mutability.IMMUTABLE, "abc")), 3
        )
...
class TestSystemContextProvider(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_build_returns_system_instructions(self):
        ...
        # System instructions should be present, plus Discord mention
    ...
    async def test_build_returns_empty_when_no_system_instructions(self):
        provider = SystemContextProvider()
        context = PromptBuildContext(apply_persona=True, persona_data={})
        result = await provider.build(context)
    ...
    async def test_build_assembles_all_persona_fields(self):
        ...
        context = PromptBuildContext(
            ...
            persona_data={
                ...
                # handled by SystemContextProvider
                "system_instructions": "Should not appear here",
            },
        )
        result = await provider.build(context)
        ...
        # Order should match ASSEMBLY_ORDER: core, background, beliefs,
        # language
...
class TestConversationHistoryProvider(unittest.IsolatedAsyncioTestCase):
    ...
    def test_name_and_priority(self):
        ...
        self.assertEqual(
            provider.priority, ProviderPriority.CONVERSATION_HISTORY
        )
    ...
    async def test_build_returns_empty_when_missing_context(self):
        ...
        result = await provider.build(
            PromptBuildContext(apply_persona=True, user_id=1, channel_id=1)
        )
        self.assertTrue(result.is_empty)

        # Missing user_id
        result = await provider.build(
            PromptBuildContext(apply_persona=True, guild_id=1, channel_id=1)
        )
        self.assertTrue(result.is_empty)
    ...
    async def test_build_returns_empty_when_apply_persona_false(self):
        provider = ConversationHistoryProvider()
        context = PromptBuildContext(
            apply_persona=False, guild_id=123, user_id=456, channel_id=789
        )
        result = await provider.build(context)
        self.assertTrue(result.is_empty)
...
class TestUserMemoryProvider(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_build_delegates_to_memory_module(self, mock_get_facts):
        ...
        provider = UserMemoryProvider()
        context = PromptBuildContext(
            apply_persona=True, guild_id=123, user_id=456, username="Alice"
        )
        result = await provider.build(context)
    ...
    async def test_build_returns_empty_when_missing_ids(self):
        ...
        # Missing guild_id
        result = await provider.build(
            PromptBuildContext(apply_persona=True, user_id=1)
        )
        self.assertTrue(result.is_empty)

        # Missing user_id
        result = await provider.build(
            PromptBuildContext(apply_persona=True, guild_id=1)
        )
        self.assertTrue(result.is_empty)
    ...
    async def test_build_returns_empty_when_apply_persona_false(self):
        provider = UserMemoryProvider()
        context = PromptBuildContext(
            apply_persona=False, guild_id=123, user_id=456, username="Alice"
        )
        result = await provider.build(context)
        self.assertTrue(result.is_empty)
...
class TestCharacterMemoryProvider(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_build_returns_empty_when_missing_ids(self):
        ...
        # Missing persona_id
        result = await provider.build(
            PromptBuildContext(apply_persona=True, guild_id=1, user_id=1)
        )
        self.assertTrue(result.is_empty)
...
class TestPersonaKnowledgeBaseProvider(unittest.IsolatedAsyncioTestCase):
    ...
    def test_name_and_priority(self):
        ...
        self.assertEqual(
            provider.priority, ProviderPriority.PERSONA_KNOWLEDGE_BASE
        )
        self.assertEqual(provider.mutability, Mutability.IMMUTABLE)
...
class TestBuildSystemPromptBackwardsCompat(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        os.environ["CANON_FILE_PATH"] = self.temp_db_path

        import utils.canon as canon_module

        canon_module.CANON_FILE_PATH = self.temp_db_path

        await canon_module.init_db()
    ...
    @patch(
        "utils.persona.PERSONA_DATA",
        {
            "system_instructions": "Global sys",
            "core_personality": "Global core",
        },
    )
    @patch("utils.memory.get_user_facts_prompt")
    async def test_uses_global_persona_data_when_none_provided(
        self, mock_get_facts
    ):
        mock_get_facts.return_value = ""
    ...
    async def test_uses_provided_persona_data(self, mock_get_facts):
        ...
        result = await build_system_prompt(
            ...
            apply_persona=True,
            persona_data={
                "system_instructions": "Provided sys",
                "core_personality": "Provided core",
            },
        )
...
class TestPromptBuilderInspect(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        """Set up temporary databases for each test."""
        # Canon database
        with tempfile.NamedTemporaryFile(
            suffix=".db", delete=False
        ) as temp_db:
            self.canon_db_path = temp_db.name
        os.environ["CANON_FILE_PATH"] = self.canon_db_path

        import utils.canon as canon_module

        canon_module.CANON_FILE_PATH = self.canon_db_path
        await canon_module.init_db()
        ...
        with tempfile.NamedTemporaryFile(
            ...
            self.memory_db_path = temp_db.name
        os.environ["MEMORY_FILE_PATH"] = self.memory_db_path

        import utils.memory as memory_module

        memory_module.MEMORY_FILE_PATH = self.memory_db_path
        await memory_module.init_db()

        # Character memory database
        with tempfile.NamedTemporaryFile(
            suffix=".db", delete=False
        ) as temp_db:
            self.character_memory_db_path = temp_db.name
        os.environ["CHARACTER_MEMORY_FILE_PATH"] = (
            self.character_memory_db_path
        )

        import utils.character_memory as char_memory_module

        char_memory_module.CHARACTER_MEMORY_FILE_PATH = (
            self.character_memory_db_path
        )
        await char_memory_module.init_db()
    ...
    async def test_inspect_returns_all_providers_with_context_blocks(self):
        ...
        for name, block in result.items():
            self.assertIsInstance(
                block, ContextBlock, f"{name} should be ContextBlock"
            )
            ...
            self.assertEqual(
                block.priority,
                getattr(ProviderPriority, priority_attr_map[name]),
            )
        ...
        self.assertEqual(
            result["character_memory"].mutability, Mutability.MUTABLE
        )
        self.assertEqual(result["guild_world"].mutability, Mutability.MUTABLE)
        self.assertEqual(
            result["persona_knowledge_base"].mutability, Mutability.IMMUTABLE
        )
        self.assertEqual(
            result["conversation_history"].mutability, Mutability.MUTABLE
        )
...
class TestTokenBudgetEnforcement(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_build_respects_token_budget_within_limit(self):
        ...
        class SmallProvider(ContextProvider):
            ...
            async def build(self, context):
                return ContextBlock(
                    self._name, self._priority, self._mutability, self._content
                )

        # 4 chars = ~1 token, so 100 chars = ~25 tokens
        providers = [
            SmallProvider("immutable1", 10, Mutability.IMMUTABLE, "x" * 100),
            # ~25 tokens
            SmallProvider("mutable1", 30, Mutability.MUTABLE, "x" * 100),
            # ~25 tokens
            SmallProvider("mutable2", 40, Mutability.MUTABLE, "x" * 100),
            # ~25 tokens
        ]
    ...
    async def test_build_drops_mutable_blocks_in_reverse_priority_order(self):
        ...
        providers = [
            SmallProvider("immutable1", 10, Mutability.IMMUTABLE, "A" * 100),
            # ~25 tokens
            SmallProvider("mutable_low", 30, Mutability.MUTABLE, "B" * 100),
            # ~25 tokens
            SmallProvider("mutable_mid", 40, Mutability.MUTABLE, "C" * 100),
            # ~25 tokens
            SmallProvider("mutable_high", 50, Mutability.MUTABLE, "D" * 100),
            # ~25 tokens
            SmallProvider("immutable2", 60, Mutability.IMMUTABLE, "E" * 100),
            # ~25 tokens
        ]
    ...
    async def test_build_never_drops_immutable_blocks(self):
        ...
        providers = [
            SmallProvider("immutable1", 10, Mutability.IMMUTABLE, "A" * 100),
            # ~25 tokens
            SmallProvider("immutable2", 20, Mutability.IMMUTABLE, "B" * 100),
            # ~25 tokens
            SmallProvider("immutable3", 25, Mutability.IMMUTABLE, "C" * 100),
            # ~25 tokens
            SmallProvider("mutable1", 30, Mutability.MUTABLE, "D" * 100),
            # ~25 tokens
        ]
    ...
    async def test_build_with_placeholder_mutability(self):
        ...
        providers = [
            ...
            SmallProvider(
                "placeholder", 20, Mutability.PLACEHOLDER, "B" * 100
            ),
