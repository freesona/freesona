...
class TestCanonComponentCRUD(unittest.TestCase):
    ...
    def test_store_and_get_component(self):
        ...
        async def test():
            ...
            self.assertEqual(
                retrieved.component_type, CanonComponentType.CORE_IDENTITY
            )
                ...
                if expected_type in inserted_types:
                    ...
                    for prev_type in expected_order[:i]:
                        if prev_type in inserted_types:
                            ...
                            self.assertLess(prev_idx, actual_idx, f"{
                                prev_type.value} should come before {
                                expected_type.value}")
    ...
    def test_get_components_by_type(self):
        ...
        async def test():
            ...
            rules = await get_components_by_type(
                TEST_PERSONA_ID, CanonComponentType.BEHAVIORAL_RULES
            )
            self.assertEqual(len(rules), 1)
            self.assertEqual(
                rules[0].component_type, CanonComponentType.BEHAVIORAL_RULES
            )

            # Get only CORE_IDENTITY
            identity = await get_components_by_type(
                TEST_PERSONA_ID, CanonComponentType.CORE_IDENTITY
            )
            self.assertEqual(len(identity), 1)
            self.assertEqual(
                identity[0].component_type, CanonComponentType.CORE_IDENTITY
            )
    ...
    def test_restore_snapshot(self):
        ...
        async def test():
            ...
            component = await get_component(
                f"restore-test-{CanonComponentType.CORE_IDENTITY.value}"
            )
            ...
            # Verify restored
            restored = await get_component(
                f"restore-test-{CanonComponentType.CORE_IDENTITY.value}"
            )
            assert restored is not None
            self.assertEqual(restored.content, f"Original {
                CanonComponentType.CORE_IDENTITY.value}")
        ...
        async def test():
            ...
            for comp_type in [
                ...
                component = CanonComponent(
                    ...
                    explanation=(
                        "The reason why"
                        if comp_type == CanonComponentType.BEHAVIORAL_RULES
                        else ""
                    ),
                    version="1.0.0",
                    author="test",
...
class TestCanonValidation(unittest.TestCase):
    ...
    def test_validate_warns_for_suspicious_content(self):
        ...
        self.assertTrue(
            any("Canonical Truth Invariant" in w for w in warnings)
        )
...
class TestCanonExportImport(unittest.TestCase):
    ...
    def test_export_import_canon(self):
        ...
        async def test():
            ...
            count = await import_canon(
                exported, target_persona_id=new_persona, author="importer"
            )
...
class TestCanonConstants(unittest.TestCase):
    ...
    def test_constants_defined(self):
        ...
        self.assertEqual(
            COMPONENT_ASSEMBLY_ORDER[0], CanonComponentType.CORE_IDENTITY
        )
        self.assertEqual(
            COMPONENT_ASSEMBLY_ORDER[-1], CanonComponentType.CANON_EXPLANATIONS
        )
...
class TestCanonSnapshotSerialization(unittest.TestCase):
    ...
    def test_snapshot_to_dict(self):
        ...
        async def test():
            ...
            snapshot = await create_snapshot(
                TEST_PERSONA_ID, "1.0.0", "test", "Test snap"
            )
            d = snapshot.to_dict()
