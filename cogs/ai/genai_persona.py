# cogs/ai/genai_persona.py: Python module.
import discord
...
class GenAIPersonaCog(commands.Cog):
    ...
    @app_commands.command(
        name="setpersona", description="Open the persona editor (Owner only)."
    )
    ...
    @commands.hybrid_command(
        name="personalock",
        aliases=["plock"],
        help="Lock the persona to prevent changes (Owner only).",
    )
    @commands.is_owner()
    async def persona_lock(self, ctx: commands.Context):
        ...
        await ctx.send("Persona locked.", ephemeral=bool(ctx.interaction))

    @commands.hybrid_command(
        name="personaunlock",
        aliases=["pulock"],
        help="Unlock the persona (Owner only).",
    )
    @commands.is_owner()
    async def persona_unlock(self, ctx: commands.Context):
        ...
        await ctx.send("Persona unlocked.", ephemeral=bool(ctx.interaction))
    ...
    @commands.hybrid_command(
        name="personasave",
        aliases=["psave"],
        help="Save current persona as a named profile (Owner only).",
    )
    @commands.is_owner()
    ...
    @commands.hybrid_command(
        name="personaload",
        aliases=["pload"],
        help="Load a saved persona profile (Owner only).",
    )
    @commands.is_owner()
    async def persona_load(self, ctx: commands.Context, name: str):
        ...
        if p.PERSONA_LOCKED:
            await ctx.send(
                "Persona is locked.", ephemeral=bool(ctx.interaction)
            )
            return
        profiles = load_profiles()
        key = name.lower()
        ...
        await ctx.send(
            f"Loaded persona `{key}`.", ephemeral=bool(ctx.interaction)
        )

    @commands.hybrid_command(
        name="personalist",
        aliases=["plist"],
        help="List saved persona profiles.",
    )
    @commands.is_owner()
    async def persona_list(self, ctx: commands.Context):
        ...
        await ctx.send(names, ephemeral=bool(ctx.interaction))

    @commands.hybrid_command(
        name="personadelete",
        aliases=["pdel"],
        help="Delete a saved persona profile (Owner only).",
    )
    @commands.is_owner()
    async def persona_delete(self, ctx: commands.Context, name: str):
        ...
        save_profiles(profiles)
        await ctx.send(
            f"Deleted profile `{key}`.", ephemeral=bool(ctx.interaction)
        )
    ...
    @commands.hybrid_command(
        name="debugpersona",
        aliases=["pdeb"],
        help="Show active persona and last prompt (Owner only).",
    )
    @commands.is_owner()
    async def debug_persona(self, ctx: commands.Context):
        ...
        last = LAST_DEBUG.get(
            ctx.channel.id, "*(no prompt sent in this channel yet)*"
        )
    ...
    @commands.hybrid_command(
        name="personareload",
        aliases=["preload"],
        help="Reload persona from persona.json (Owner only).",
    )
    @commands.is_owner()
    async def persona_reload(self, ctx: commands.Context):
        _persona, legacy = reload_persona()
        status = " (legacy mode)" if legacy else ""
        await ctx.send(
            f"Persona reloaded from `persona.json`.{status}",
            ephemeral=bool(ctx.interaction),
        )
