...
class ChromaCog(commands.Cog):
    ...
    @commands.hybrid_command(name="kbadd", help="Add a note or attached PDF/EPUB file to the local knowledge base.")
    @commands.has_permissions(administrator=True)
    async def kbadd(
        self,
        ...
        attachment = attachment or (ctx.message.attachments[0] if getattr(ctx, "message", None) and ctx.message.attachments else None)
        ...
        if attachment is not None:
            ...
            # Extract text off-thread for CPU-heavy parsing (PDFs, EPUBs)
            ...
            if not extracted.strip():
                await ctx.send(
                    "Could not extract readable text from that file. Supported input includes PDF and EPUB files.",
                    ephemeral=True,
                )
                return
        ...
        if not text_parts:
            await ctx.send("Provide text or attach a supported PDF/EPUB file.", ephemeral=True)
            return
