# cogs/moderation/core.py: Moderation actions like ban, timeout, and kick
# commands.
...
class ModerationCog(commands.Cog):
    ...
    async def kick_cmd(
        ...
        *,
        ...
        dm_embed = discord.Embed(
            title="You have been kicked", color=discord.Color.orange()
        )
        ...
        try:
            ...
            await ctx.send(
                f"**{member}** has been kicked. Reason: {reason}{note}"
            )
    ...
    @commands.hybrid_command(
        name="purge",
        help="Deletes a specified number of messages (1-1000).",
        usage="<amount>",
    )
    ...
    async def purge_cmd(self, ctx, amount: int):
        ...
        if 1 <= amount <= 1000:
            try:
                ...
                await ctx.send(
                    f"Poof! Deleted {len(deleted) - 1} message(s).",
                    delete_after=5,
                )
    ...
    @commands.hybrid_command(
        ...
        usage="<member> [delete_messages] [reason]",
    )
    @app_commands.describe(
    ...
    async def ban_cmd(
        ...
        *,
        ...
        if target:
            ...
            dm_embed = discord.Embed(
                title="You have been banned", color=discord.Color.red()
            )
            dm_embed.add_field(
        ...
        delta = (
            parse_time_string(delete_messages)
        ...
        try:
            ...
            note = "" if dm_sent else " *(couldn't DM user)*"
            del_note = (
                f", deleted messages from the past {delete_messages}"
                if delete_seconds
                else ""
            )
            await ctx.send(
    ...
    @commands.hybrid_command(
        name="softban",
        help="Bans and immediately unbans a member to purge their messages.",
        usage="<member> [delete_messages] [reason]",
    )
    @app_commands.describe(
    ...
    async def softban_cmd(
        ...
        *,
        ...
        dm_embed = discord.Embed(
            title="You have been softbanned", color=discord.Color.orange()
        )
        dm_embed.add_field(name="Server", value=ctx.guild.name, inline=False)
        dm_embed.add_field(name="Reason", value=reason, inline=False)
        dm_embed.set_footer(
    ...
    @app_commands.describe(
        user="The user to unban (ID or user#tag).",
    ...
    async def unban_cmd(
        ...
        try:
            await ctx.guild.unban(
                user, reason=f"Unbanned by {ctx.author}: {reason}"
            )
            dm_embed = discord.Embed(
                title="You have been unbanned", color=discord.Color.green()
            )
            dm_embed.add_field(
                name="Server", value=ctx.guild.name, inline=False
            )
    ...
    @commands.hybrid_command(
        name="removetimeout",
        aliases=["rt", "rto"],
        help="Removes a timeout from a member.",
        usage="<member>",
    )
    ...
    @commands.hybrid_command(
        name="slowmode",
        help="Sets the slowmode delay for the current channel.",
        usage="<duration|off>",
    )
    @app_commands.describe(
    ...
    @commands.hybrid_command(
        name="unlock", help="Unlocks the current channel.", usage="[reason]"
    )
