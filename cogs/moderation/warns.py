...
async def get_warn_count(guild_id: int, user_id: int) -> int:
    async with (
        aiosqlite.connect(WARNINGS_DB) as db,
        db.execute(
...
async def apply_threshold(ctx, member: discord.Member, warn_count: int):
    ...
    try:
        if action == "timeout":
            ...
            if delta:
                await member.timeout(
                    delta, reason=f"Auto-threshold: {warn_count} warnings"
                )
                await ctx.send(
            ...
            await ctx.guild.ban(
                member, reason=f"Auto-threshold: {warn_count} warnings"
            )
            await ctx.send(
                f"Auto-threshold reached ({warn_count} warns): "
        ...
        logger.error(
            f"Auto-threshold failed for {member} at {warn_count} warns: {e}"
        )
...
class ThresholdModal(discord.ui.Modal, title="Edit Warn Thresholds"):
    ...
    def __init__(self, current_thresholds: dict):
        ...
        self.enabled.default = (
            "yes" if current_thresholds.get("enabled", True) else "no"
        )

    async def on_submit(self, interaction: discord.Interaction):
        is_enabled = self.enabled.value.strip().lower() in (
            "yes",
            "y",
            "true",
            "1",
...
class WarnsCog(commands.Cog):
    ...
    @commands.hybrid_command(
        name="warn",
        help="Issues a warning to a member.",
        usage="<member|id> [reason]",
    )
    @app_commands.describe(
        member="The member to warn (mention, username, or ID).",
        reason="Reason for the warning.",
    )
    @commands.has_permissions(moderate_members=True)
    @commands.bot_has_permissions(moderate_members=True)
    async def warn_cmd(
        self, ctx, member: str, *, reason: str = "No reason provided"
    ):
        await ctx.defer()
        ...
        if guild_member.bot:
            ...
            return
        if (
            guild_member.top_role >= ctx.author.top_role
        ...
        dm_embed = discord.Embed(
            title="You have been warned", color=discord.Color.yellow()
        )
    ...
    @commands.hybrid_command(
        name="warns",
        help="Shows all warnings for a member.",
        usage="<member|id>",
    )
    @app_commands.describe(
        member="The member to check warnings for (mention, username, or ID)."
    )
    @commands.has_permissions(moderate_members=True)
    async def warns_cmd(self, ctx, member: str):
        ...
        embed = discord.Embed(
            title=f"Warnings for {user}", color=discord.Color.orange()
        )
        ...
        for row in rows:
            ...
            ts = datetime.fromisoformat(row["timestamp"]).strftime(
                "%Y-%m-%d %H:%M UTC"
            )
            embed.add_field(
    ...
    @commands.hybrid_command(
        name="delwarn", help="Deletes a warning by its ID.", usage="<warn_id>"
    )
    @app_commands.describe(
        warn_id="The warning ID to delete (get it from /warns)."
    )
    @commands.has_permissions(moderate_members=True)
    async def delwarn_cmd(self, ctx, warn_id: str):
        ...
        async with aiosqlite.connect(WARNINGS_DB) as db:
            ...
            if not row:
                await ctx.send(
                    f"No warning with ID `{warn_id}` found in this server."
                )
                return

            await db.execute(
                "DELETE FROM warnings WHERE warn_id = ?", (warn_id.lower(),)
            )
            await db.commit()
        ...
        await ctx.send(
            f"Warning `{warn_id}` for **{member_str}** has been deleted."
        )

    @commands.hybrid_command(
        name="clearwarns",
        help="Clears all warnings for a member.",
        usage="<member|id>",
    )
    @app_commands.describe(
        member=(
    ...
    @commands.hybrid_command(
        name="warnthresholds",
        aliases=["wt"],
        help="View or edit auto-punishment thresholds for warnings.",
        usage="",
    )
