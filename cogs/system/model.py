...
from utils.config_schema import MODEL_CHOICES
...
class ModelCog(commands.Cog):
    ...
    @commands.hybrid_group(
        name="model", help="Show or change the active model."
    )
    @commands.is_owner()
    ...
    @model_group.command(
        name="show", help="Show the active provider and model."
    )
    @commands.is_owner()
    ...
    # type: ignore[attr-defined]
    @model_group.command(
        name="reset", help="Reset the model to the environment/default value."
    )
    @commands.is_owner()
    async def model_reset(self, ctx: commands.Context):
        ...
        config = get_provider_config()
        await ctx.send(
            f"Model reset to `{config['model']}`.",
            ephemeral=bool(ctx.interaction),
        )

    # type: ignore[attr-defined]
    @model_group.command(
        name="temperature", help="Set the model temperature (0.0-2.0)."
    )
    @commands.is_owner()
    ...
    async def model_temperature(self, ctx: commands.Context, value: float):
        ...
        save_config(config)
        await ctx.send(
            f"Model temperature set to `{value}`.",
            ephemeral=bool(ctx.interaction),
        )

    # type: ignore[attr-defined]
    @model_group.command(
        name="temperature_reset", help="Reset temperature to default (0.7)."
    )
    @commands.is_owner()
    async def model_temperature_reset(self, ctx: commands.Context):
        ...
        save_config(config)
        await ctx.send(
            "Model temperature reset to default (0.7).",
            ephemeral=bool(ctx.interaction),
        )
