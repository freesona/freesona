...
class CanonComponent:
    ...
    content: str
    explanation: str = ""  # The "why" — required for behavioral_rules, optional for others
    version: str = "1.0.0"
    author: str = "system"
...
async def get_component(component_id: str) -> Optional[CanonComponent]:
...
async def get_components_by_type(persona_id: str, component_type: CanonComponentType) -> List[CanonComponent]:
    ...
    async with aiosqlite.connect(_get_canon_file_path()) as db:
        ...
        """, (persona_id, component_type.value)) as cursor:
            rows = await cursor.fetchall()
            return [CanonComponent.from_row(row) for row in rows]
    ...
    async with aiosqlite.connect(_get_canon_file_path()) as db:
        ...
        """, (persona_id, limit)) as cursor:
            ...
            for row in rows:
                ...
                snapshots.append(CanonSnapshot(
...
async def ensure_default_canon(persona_id: str, author: str = "system") -> None:
    ...
    await create_snapshot(persona_id, _get_canon_version(), author, "Initial canon creation")
...
def validate_canon_component(component: CanonComponent) -> List[str]:
    ...
    if component.component_type == CanonComponentType.BEHAVIORAL_RULES:
        if not component.explanation or not component.explanation.strip():
    ...
    prohibited_patterns = [
        "we promised", "we agreed", "we decided", "we experienced",
        "the user", "the user said", "the user likes", "the user dislikes",
        "conversation", "chat history", "recently", "last time",
        "in this server", "in this channel", "guild", "discord",
    ]
...
async def import_canon(data: Dict[str, Any], target_persona_id: str, author: str = "import") -> int:
    ...
    await create_snapshot(target_persona_id, _get_canon_version(), author, f"Imported {count} components")
