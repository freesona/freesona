# main.py: Main code that loads the cogs and makes the bot run. Also
# contains the prefix command and the on_ready event that sends a message
...
@bot.hybrid_command(
    name="prefix",
    description="Changes the bot prefix and saves it to config.json",
)
...
async def on_ready():
    ...
    try:
        from cogs.ai.genai import BOT_NAME as GENAI_BOT_NAME
        from cogs.ai.genai import LEGACY_DETECTED
...
async def main():
    await asyncio.gather(start_http(), start_bot())
